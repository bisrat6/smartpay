...
const PaymentsManagement = () => {
  ...
  const handleApproveSingle = async (paymentId: string) => {
    setLoading(true);
    try {
      const response = await paymentApi.approveSingle(paymentId);
  ...
  const handleBulkApprove = async () => {
    ...
    try {
      ...
      } else if (successful === approved) {
        toast.success(`All ${approved} payments approved and processed successfully!`);
      } else if (successful > 0) {
        toast.warning(`Bulk approval completed: ${approved} approved, ${successful} processed successfully, ${failed} failed`);
      } else {
        toast.error(`All ${approved} payments approved but failed to process. Please check your Arifpay configuration.`);
      }
      ...
      const errorMessage = error?.response?.data?.message || error?.message || 'Failed to approve payments';
      const errorDetails = error?.response?.data?.details;
